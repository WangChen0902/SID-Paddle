...
data_prefix = '/root/paddlejob/workspace/train_data/datasets/'
output_prefix = '/root/paddlejob/workspace/output/'

input_dir = os.path.join(data_prefix, 'data/Sony/short/')
gt_dir = os.path.join(data_prefix, 'data/Sony/long/')
checkpoint_dir = os.path.join(output_prefix, 'result_Sony/')  # path to save ckeckpoints
result_dir = os.path.join(output_prefix, 'result_Sony/')  # path to save result images
...
if train_parameters['start_epoch'] != 0:
    model_name = checkpoint_load_dir + 'model_%04d.pdparams' % train_parameters['start_epoches']
    opt_name = checkpoint_load_dir + 'optimizer_%04d.pdopt' % train_parameters['start_epoches']
...
for epoch in range(train_parameters['start_epoches'], train_parameters['num_epoches']):
    cnt = 0
    # print(cnt)
    if epoch > 2000:
    ...
    for ind in np.random.permutation(len(train_ids)):
        ...
        if epoch % save_freq == 0:
            ...
            paddle.save(net_model.state_dict(), checkpoint_dir + 'model_%04d.pdparams' % epoch)
            paddle.save(optimizer.state_dict(), checkpoint_dir + 'optimizer_%04d.pdopt' % epoch)
